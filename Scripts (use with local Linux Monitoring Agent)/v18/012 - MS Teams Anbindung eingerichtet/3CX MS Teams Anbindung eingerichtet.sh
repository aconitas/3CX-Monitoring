...
RESULT=$(curl --request GET --cookie /tmp/3cxcookie --cookie-jar /tmp/3cxcookie -s localhost:${HTTP_PORT}/api/ExtensionList/IsTeamsEnabled)

########################################################
#### Auswertung
if [[ $RESULT =~ 'false' ]]; then
  echo MS Teams Anbindung NICHT eingerichtet
  exit 0
elif [[ $RESULT =~ 'true' ]]; then
  echo MS Teams Anbindung eingerichtet
  exit 2
else
